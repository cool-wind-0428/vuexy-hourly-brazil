...
// GET: Return Users
// ------------------------------------------------
mock.onGet('/apps/app/apps').reply(config => {
  // eslint-disable-next-line object-curly-newline
  const { q = '', perPage = 10, page = 1, sortBy = 'id', sortDesc = false, status = null } = config.params
  /* eslint-enable */

  const queryLowered = q.toLowerCase()
  const filteredData = data.apps.filter(
    app =>
      /* eslint-disable operator-linebreak, implicit-arrow-linebreak */
      (app.client.companyEmail.toLowerCase().includes(queryLowered) ||
        app.client.name.toLowerCase().includes(queryLowered)) &&
      app.appStatus === (status || app.appStatus),
  )
  /* eslint-enable  */
  ...
  return [
    200,
    {
      apps: paginateArray(sortedData, perPage, page),
...
// ------------------------------------------------
// GET: Return Single app
// ------------------------------------------------
mock.onGet(/\/apps\/app\/apps\/\d+/).reply(config => {
  // Get event id from URL
  let appId = config.url.substring(config.url.lastIndexOf('/') + 1)

  // Convert Id to number
  appId = Number(appId)

  const appIndex = data.apps.findIndex(e => e.id === appId)
  ...
  if (app) return [200, responseData]
  return [404]
})
...
// GET: Return Clients
// ------------------------------------------------
mock.onGet('/apps/app/clients').reply(() => {
  const clients = data.apps.map(app => app.client)
